...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (cep.length === 8) {
                try {
                    ...
                    if (!dados.erro) {
                        ...
                        if(document.getElementById('edit-numero')) document.getElementById('edit-numero').focus();
                    }
                } catch (e) {}
            }
        });
    ...
    function renderizarProntuario() {
        if (!pacienteAtual) {
            ...
            if(document.getElementById('detalhe-txt-telefone')) document.getElementById('detalhe-txt-telefone').textContent = "(--) ----- -----";
            if(document.getElementById('detalhe-txt-email')) document.getElementById('detalhe-txt-email').textContent = "sem-email@sistema.com";
            if(document.getElementById('detalhe-txt-endereco')) document.getElementById('detalhe-txt-endereco').textContent = "Endereço não informado";
            return;
        }
        ...
        if(document.getElementById('detalhe-txt-telefone')) {
            document.getElementById('detalhe-txt-telefone').textContent = pacienteAtual.telefone || "Não informado";
        }
        
        if(document.getElementById('detalhe-txt-email')) {
            document.getElementById('detalhe-txt-email').textContent = pacienteAtual.email || "Não informado";
        }
        
        if(document.getElementById('detalhe-txt-endereco')) {
            const logradouro = pacienteAtual.logradouro || "Endereço não informado";
            const numero = pacienteAtual.numero ? `, Nº ${pacienteAtual.numero}` : '';
            const bairro = pacienteAtual.bairro ? ` - ${pacienteAtual.bairro}` : '';
            const cidade = pacienteAtual.cidade ? `, ${pacienteAtual.cidade}` : '';
            const estado = pacienteAtual.estado ? `/${pacienteAtual.estado}` : '';
            const cep = pacienteAtual.cep ? ` (CEP: ${pacienteAtual.cep})` : '';

            document.getElementById('detalhe-txt-endereco').textContent = `${logradouro}${numero}${bairro}${cidade}${estado}${cep}`;
        }
    ...
    if (btnConcluirAgendar) {
        btnConcluirAgendar.addEventListener('click', () => {
            ...
            if (!vagaSelecionadaDados) {
                alert("Ação recusada! Por favor, clique sobre uma vaga activa listada acima antes de confirmar.");
                return;
            }
