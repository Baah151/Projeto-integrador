...
window.addEventListener('DOMContentLoaded', () => {
    // VARIÁVEIS DE CONTAGEM ZERADAS AGUARDANDO RETORNO DO JAVA REST
    ...
    // CORRIGIDO: Vinculado ao ID correto do menu lateral ('badge-painel')
...
function mostrarIaEmEspera(container) {
    container.innerHTML = `
        <div class="ai-card empty-ai">
            <p>✨ <strong>IA em espera:</strong> Assim que os pacientes realizarem os primeiros cadastros, eu começarei a fornecer sugestões e alertas clínicos aqui.</p>
        </div>
    `;
}
        ...
        card.querySelector('.btn-close-ai').addEventListener('click', function() {
            ...
            setTimeout(() => {
                ...
                if (container.children.length === 0) {
                    container.innerHTML = `
                        <div class="ai-card empty-ai">
                            <p>✨ <strong>IA em espera:</strong> Sem sugestões ou novos alertas clínicos pendentes no momento.</p>
                        </div>
                    `;
                }
            }, 300);
