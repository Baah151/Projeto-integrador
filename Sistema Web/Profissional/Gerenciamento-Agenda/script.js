window.addEventListener('DOMContentLoaded', () => {
    ...
    // LISTAS INICIAIS DA AGENDA ORIGINAL
    let horariosTemporarios = {};
    let agendaPublicada = [];

    // Recupera dados simulados locais para testes visuais do protótipo
    const agendasSalvas = localStorage.getItem('agendaPublicadaProfissional');
    if (agendasSalvas) {
        agendaPublicada = JSON.parse(agendasSalvas);
    }

    // Configura o input do calendário para abrir no dia de hoje automaticamente
    const hoje = new Date();
    ...
    function renderizarAgendaPublicada() {
        if (!agendaPublicadaContainer) return;
        agendaPublicadaContainer.innerHTML = '';
        
        const dataAtual = new Date();
        dataAtual.setHours(0, 0, 0, 0);

        // Filtra a exibição para remover da lista visível os dias que já passaram
